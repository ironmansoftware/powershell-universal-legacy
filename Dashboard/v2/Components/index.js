...
function redirectToHomePage(dashboard) {
    ...
    else if (defaultHomePage.dynamic && defaultHomePage.url != null && defaultHomePage.url.indexOf(":") === -1) {
...
class Materialize extends React.Component {
    ...
    render() {
        var { dashboard, roles } = this.props;

        var authedPages = dashboard.pages.filter(x => {
            if (!x.role) return true;
            if (!roles || roles.length == 0) return false; 
            if (!Array.isArray(roles)) return false; 
            if (roles.indexOf(x.role) == -1) return false;
            return true;
        });

        var component = this;

        var dynamicPages = authedPages.map(function (x) {
            ...
            return <Route key={x.url} path={x.url} render={props => (
        ...
        var staticPages = authedPages.map(function (x) {
            if (x.dynamic) return null;

            return <Route key={x.name} exact path={'/' + x.name.replace(/ /g, "-")} render={props => (
        ...
        <main style={{ background: dashboard.backgroundColor, color: dashboard.fontColor }}>
            <Suspense fallback={<span />}>
                <Switch>
                    {staticPages}
                    {dynamicPages}
                    <Route exact path={`/`} render={() => redirectToHomePage(dashboard)} />
...
UniversalDashboard.renderDashboard = (props) => <Materialize {...props}/>;